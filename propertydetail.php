...
$pid = isset($_GET['pid']) ? mysqli_real_escape_string($con, $_GET['pid']) : null;
$uid = mysqli_real_escape_string($con, $_SESSION['uid']);

// Fetch property details
$property_sql = "SELECT * FROM property WHERE pid = ?";
$stmt = $con->prepare($property_sql);
...
$interaction_type = 'view'; // or 'like', 'save', etc.
$log_sql = "INSERT INTO user_interactions (uid, pid, interaction_type) VALUES (?, ?, ?)";
$stmt = $con->prepare($log_sql);
$stmt->bind_param("iis", $uid, $pid, $interaction_type);
$stmt->execute();

// Fetch recommended properties based on user interactions
$recommendation_sql = "
...
$stmt = $con->prepare($recommendation_sql);
$stmt->bind_param("ii", $pid, $pid);
$stmt->execute();
$recommendation_result = $stmt->get_result();
$recommended_properties = $recommendation_result->fetch_all(MYSQLI_ASSOC);
?>
...
<div id="page-wrapper">
    ...
    <link rel="stylesheet" type="text/css" href="css/style.css?v=1.1">
        <?php foreach ($recommended_properties as $property): ?>
            <?php
                $recommended_pid = $property['pid'];
                $property_sql = "SELECT * FROM property WHERE pid = ?";
                $stmt = $con->prepare($property_sql);
                $stmt->bind_param("i", $recommended_pid);
                $stmt->execute();
                $property_result = $stmt->get_result();
                $recommended_property_details = $property_result->fetch_assoc();
            ?>
            <div class="recommended-property-tile">
